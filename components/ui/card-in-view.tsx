...
import { useCardInViewContext } from "./card-in-view-context"
...
export function CardInView({
  ...
  const { registerCard, unregisterCard } = useCardInViewContext() || {}
  ...
  useEffect(() => {
    ...
    // SI HAY CONTEXTO Y ES MOBILE: DELEGAR CONTROL
    if (registerCard && mobile && currentRef) {
      // Registrarse en el contexto
      registerCard(id, currentRef, (active) => {
        setIsInView(active)
        if (linkRef.current) {
          if (active) {
      ...
      // Limpieza solo de listeners globales que no necesitamos si delegamos
      // No necesitamos IntersectionObserver ni scroll listeners locales
      return () => {
        mobileMediaQuery.removeEventListener('change', checkMobile)
        window.removeEventListener("resize", checkMobile)
        if (unregisterCard) unregisterCard(id)
        if (linkRef.current) linkRef.current.classList.remove("group-in-view")
      }
    }

    // SI NO HAY CONTEXTO O NO ES MOBILE: COMPORTAMIENTO ORIGINAL
    // Only set up on mobile (si no hay contexto)
    if (!mobile) {
      return () => {
        mobileMediaQuery.removeEventListener('change', checkMobile)
        window.removeEventListener("resize", checkMobile)
  ...
  }, [threshold, rootMargin, registerCard, unregisterCard, id])
